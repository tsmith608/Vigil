...
export async function GET() {
  try {
    console.log("📥 Loading ONEWEB TLEs from DB...");

        const res = await pool.query(`
        SELECT name, satnum, line1, line2
        FROM public.tles
        WHERE constellation = 'oneweb'
        ORDER BY satnum
        `);
        ...
        console.log(`📄 Retrieved ${tles.length} ONEWEB TLE entries from DB`);
